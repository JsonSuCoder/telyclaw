...
```
Settings.tsx (toggle)
  → tauriApi.preventSleep.set(true/false)
...
- Init: load from `tauriApi.preventSleep.get()` alongside autoLaunch
- Toggle handler: call `tauriApi.preventSleep.set(next)`, update state
