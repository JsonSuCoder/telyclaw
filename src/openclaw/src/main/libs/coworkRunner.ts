...
import { isPackaged, getResourcesPath, getUserDataPath } from '../env';
import fs from 'fs';
...
import { getNodeRuntimePath, getEnhancedEnv, getEnhancedEnvWithTmpdir, getSkillsRoot } from './coworkUtil';
...
export class CoworkRunner extends EventEmitter {
  ...
  private async runClaudeCodeLocal(
    ...
    coworkLog('INFO', 'runClaudeCodeLocal', `MCP env: LOBSTERAI_NODE_PATH=${envVars.LOBSTERAI_NODE_PATH || '(not set)'}`);
    ...
    if (isPackaged()) {
      ...
      options.spawnClaudeCodeProcess = (spawnOptions: {
        ...
        const isPackagedDarwin = isPackaged() && process.platform === 'darwin';
        ...
        if (useElectronShim) {
          spawnEnv.LOBSTERAI_NODE_PATH = spawnOptions.env?.LOBSTERAI_NODE_PATH || nodeRuntimePath;
        } else {
          delete spawnEnv.LOBSTERAI_NODE_PATH;
        }
        ...
        if (process.platform === 'win32' && isNodeLikeCommand) {
          ...
          spawnEnv.LOBSTERAI_NODE_PATH = nodeRuntimePath;
          coworkLog('INFO', 'runClaudeCodeLocal', `Rewrote Windows SDK command "${spawnOptions.command || 'node'}" to Electron runtime: ${nodeRuntimePath}`);
        } else if (isPackagedDarwin && isNodeLikeCommand) {
          command = nodeRuntimePath;
          spawnEnv.LOBSTERAI_NODE_PATH = nodeRuntimePath;
          coworkLog('INFO', 'runClaudeCodeLocal', `Rewrote packaged macOS SDK command "${spawnOptions.command || 'node'}" to Electron helper runtime: ${nodeRuntimePath}`);
        }
              ...
              case 'stdio':
                {
                  ...
                  if (process.platform === 'win32' && isPackaged() && effectiveStdioCommand) {
                    ...
                    const withElectronNodeEnv = (base: Record<string, string> | undefined): Record<string, string> => ({
                      ...(base || {}),
                      ELECTRON_RUN_AS_NODE: '1',
                      LOBSTERAI_NODE_PATH: nodeRuntimePath,
                    });
                    ...
                    if (
                      ...
                      effectiveStdioCommand = nodeRuntimePath;
                  ...
                  if (isPackaged() && process.platform === 'darwin' && stdioCommand && path.isAbsolute(stdioCommand)) {
                    ...
                    try {
                      appExecCandidates.add(fs.realpathSync.native(nodeRuntimePath));
                    } catch {
                      // Ignore realpath resolution errors.
                    }
