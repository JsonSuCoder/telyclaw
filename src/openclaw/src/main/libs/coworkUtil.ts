import { getUserDataPath, getResourcesPath, isPackaged, getAppName } from '../env';
...
export function getNodeRuntimePath(): string {
...
export function ensureNodeShim(electronPath: string, npmBinDir?: string): string | null {
  try {
...
function applyPackagedEnvOverrides(env: Record<string, string | undefined>): void {
  const electronNodeRuntimePath = getNodeRuntimePath();

  if (isPackaged() && !env.LOBSTERAI_ELECTRON_PATH) {
  ...
  if (shouldInjectShim) {
    const shimDir = ensureNodeShim(electronNodeRuntimePath, npmBinDir);
    if (shimDir) {
...
export async function getEnhancedEnv(target: OpenAICompatProxyTarget = 'local'): Promise<Record<string, string | undefined>> {
  ...
  if (process.platform === 'win32' || env.LOBSTERAI_NODE_SHIM_ACTIVE === '1') {
    env.LOBSTERAI_ELECTRON_PATH = getNodeRuntimePath().replace(/\\/g, '/');
  } else {
