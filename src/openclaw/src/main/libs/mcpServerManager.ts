...
import { getNodeRuntimePath, getEnhancedEnv } from './coworkUtil';
...
function ensureWindowsHideInitScript(): string | null {
  ...
  try {
    const dir = path.join(getUserDataPath(), 'mcp-bridge', 'bin');
...
async function resolveStdioCommand(server: McpServerRecord): Promise<ResolvedStdioCommand> {
  ...
  if (process.platform === 'win32' && isPackaged() && effectiveCommand) {
    ...
    if (nodeCommandType) {
      ...
      if (systemNode) {
        ...
        if (nodeCommandType === 'node') {
          effectiveCommand = nodeRuntimePath;
  ...
  if (isPackaged() && process.platform === 'darwin' && stdioCommand && path.isAbsolute(stdioCommand)) {
    ...
    const appExecCandidates = new Set([
      ...
      nodeRuntimePath, path.resolve(nodeRuntimePath),
    ]);
    ...
    try { appExecCandidates.add(fs.realpathSync.native(nodeRuntimePath)); } catch { /* ignore */ }
