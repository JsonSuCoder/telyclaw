...
import { getNodeRuntimePath, ensureNodeShim, getSkillsRoot } from './coworkUtil';
...
export class OpenClawEngineManager extends EventEmitter {
  ...
  private async doStartGateway(): Promise<OpenClawEngineStatus> {
    ...
    const env: NodeJS.ProcessEnv = {
      ...
      LOBSTERAI_NODE_PATH: nodeRuntimePath.replace(/\\/g, '/'),
    ...
    const npmBinDir = isPackaged()
      ? path.join(getResourcesPath(), 'app.asar.unpacked', 'node_modules', 'npm', 'bin')
      : undefined;
    const nodeShimDir = ensureNodeShim(electronNodeRuntimePath, npmBinDir);
    if (nodeShimDir) {
  ...
  private resolveRuntimeMetadata(): RuntimeMetadata {
    const candidateRoots = isPackaged()
      ? [path.join(getResourcesPath(), 'cfmind')]
      : [
    ...
    const expectedPathHint = isPackaged()
      ? path.join(getResourcesPath(), 'cfmind')
      : path.join(process.cwd(), 'vendor', 'openclaw-runtime', 'current');
