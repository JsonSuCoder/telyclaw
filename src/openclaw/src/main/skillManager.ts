import { getUserDataPath, getResourcesPath, isPackaged, getAppVersion } from '../env';
...
function buildSkillEnv(): Record<string, string | undefined> {
  ...
  env.LOBSTERAI_ELECTRON_PATH = getNodeRuntimePath();
...
const ensureWindowsHideScript = (): string | null => {
  ...
  try {
    const dir = path.join(getUserDataPath(), 'bin');
...
const resolveNpxCliJs = (): string | null => {
  const candidates = isPackaged()
    ? [path.join(getResourcesPath(), 'app.asar.unpacked', 'node_modules', 'npm', 'bin', 'npx-cli.js')]
    : [
...
const downloadClawhubSkill = async (
  ...
  const electronPath = getNodeRuntimePath();
...
const resolveNpmCliJs = (): string | null => {
  const candidates = isPackaged()
    ? [path.join(getResourcesPath(), 'app.asar.unpacked', 'node_modules', 'npm', 'bin', 'npm-cli.js')]
    : [
...
export class SkillManager {
  ...
  private notifySkillsChanged(): void {
    console.log('[skills] skills changed, notify refresh');
