...
import fs from 'fs';
import { getUserDataPath, isPackaged, getResourcesPath } from '../env';
import * as os from 'os';
import { cpRecursiveSync } from './fsCompat';
import { getNodeRuntimePath } from './libs/coworkUtil';
...
function buildSkillServiceEnv(): Record<string, string | undefined> {
  ...
  if (isPackaged()) {
    ...
    env.LOBSTERAI_NODE_PATH = nodeRuntimePath;
  ...
  // Expose Node runtime executable so skill scripts can run JS
  // even when system Node.js is not installed.
  // (In packaged mode, this is set to the internal Node runtime path)
  if (!isPackaged()) {
    env.LOBSTERAI_NODE_PATH = nodeRuntimePath;
  }
...
export class SkillServiceManager {
  ...
  private repairWebSearchRuntimeFromBundled(skillPath: string): void {
    ...
    const candidates = [
      path.join(getResourcesPath(), 'SKILLs', 'web-search'),
  ...
  private async startWebSearchServiceProcess(skillPath: string): Promise<void> {
    ...
    const runtimeLabel = runtime.command === 'node' ? 'node' : 'node-runtime';
  ...
  private getWebSearchPath(): string | null {
    ...
    if (isPackaged()) {
      ...
      candidates.push(path.join(getUserDataPath(), 'SKILLs', 'web-search'));
      candidates.push(path.join(getResourcesPath(), 'SKILLs', 'web-search'));
      candidates.push(path.join(process.cwd(), 'SKILLs', 'web-search'));
    } else {
