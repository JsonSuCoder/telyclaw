...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initializeApp = async () => {
      try {
        ...
        const entConfig = await api.enterprise.getConfig();
  ...
  useEffect(() => {
    const removeListener = api.githubCopilot.onTokenUpdated(({ token, baseUrl }) => {
  ...
  useEffect(() => {
    const handleOnline = () => {
      ...
      api.networkStatus.send('online');
    };

    const handleOffline = () => {
      console.log('[Renderer] Network offline');
      api.networkStatus.send('offline');
    };
  ...
  const runUpdateCheck = useCallback(async () => {
    try {
      const currentVersion = await api.appInfo.getVersion();
  ...
  const handleConfirmUpdate = useCallback(async () => {
    ...
    if (updateInfo.url.includes('#') || updateInfo.url.endsWith('/download-list')) {
      setShowUpdateModal(false);
      try {
        const result = await api.shell.openExternal(updateInfo.url);
        if (!result.success) {
    ...
    const unsubscribe = api.appUpdate.onDownloadProgress((progress) => {
      setDownloadProgress(progress);
    });

    try {
      const downloadResult = await api.appUpdate.download(updateInfo.url);
      unsubscribe();
      ...
      const installResult = await api.appUpdate.install(downloadResult.filePath!);
  ...
  const handleCancelDownload = useCallback(async () => {
    await api.appUpdate.cancelDownload();
    setUpdateModalState('info');
    setDownloadProgress(null);
  }, []);
  ...
  // 监听托盘菜单打开设置的 IPC 事件
  useEffect(() => {
    const unsubscribe = api.ipcRenderer.on('app:openSettings', () => {
      handleShowSettings();
    });
    return unsubscribe;
  }, [handleShowSettings]);
  ...
  // 监听托盘菜单新建任务的 IPC 事件
  useEffect(() => {
    const unsubscribe = api.ipcRenderer.on('app:newTask', () => {
      handleNewChat();
    });
    return unsubscribe;
  }, [handleNewChat]);
