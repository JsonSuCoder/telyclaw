...
const openExternalViaDefaultBrowser = async (url: string): Promise<boolean> => {
  const openExternal = api?.shell?.openExternal;
    ...
    if (isLocalFilePath) {
      ...
      const handleClick = async (e: React.MouseEvent<HTMLAnchorElement>) => {
        ...
        try {
          ...
          if (fallbackPath) {
            const fallbackResult = await api.shell.openPath(fallbackPath);
      ...
      const handleRevealInFolder = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        const tryReveal = async (targetPath: string): Promise<boolean> => {
          const result = await api.shell.showItemInFolder(targetPath);
          if (result?.success) {
            return true;
          }
...
const MarkdownContent: React.FC<MarkdownContentProps> = ({
  ...
  return (
    <div className={`markdown-content text-[15px] leading-6 overflow-hidden break-all ${className}`}>
      <ReactMarkdown
