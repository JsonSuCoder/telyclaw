...
const Settings: React.FC<SettingsProps> = ({ onClose, initialTab, notice, noticeI18nKey, noticeExtra, onUpdateFound, enterpriseConfig }) => {
  ...
  const handleOpenUserCommunity = useCallback(() => {
    void api.shell.openExternal(ABOUT_USER_COMMUNITY_URL);
  }, []);

  const handleOpenServiceTerms = useCallback(() => {
    void api.shell.openExternal(ABOUT_SERVICE_TERMS_URL);
  }, []);
  ...
  const handleExportLogs = useCallback(async () => {
    ...
    try {
      const result = await api.log.exportZip();
      if (!result.success) {
  ...
  useEffect(() => {
    try {
      ...
      api.autoLaunch.get().then(({ enabled }) => {
      ...
      api.preventSleep.get().then(({ enabled }) => {
  ...
  const handleMiniMaxDeviceLogin = async (region: MiniMaxRegion) => {
    ...
    try {
      ...
      const codeRes = await api.api.fetch({
        url: codeEndpoint,
        method: 'POST',
        headers: {
      ...
      try {
        await api.shell.openExternal(codePayload.verification_uri);
      ...
      while (Date.now() < expireTimeMs) {
        ...
        const tokenRes = await api.api.fetch({
          url: tokenEndpoint,
          method: 'POST',
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
  ...
  const handleCopilotSignIn = async () => {
    try {
      ...
      const { userCode, verificationUri, deviceCode, interval, expiresIn } =
        await api.githubCopilot.requestDeviceCode();
      ...
      await api.shell.openExternal(verificationUri);

      // Step 2: Poll for token
      setCopilotAuthStatus('polling');
      const result = await api.githubCopilot.pollForToken(deviceCode, interval, expiresIn);
  ...
  const handleCopilotSignOut = async () => {
    try {
      await api.githubCopilot.signOut();
  ...
  const handleCopilotCancelAuth = async () => {
    try {
      await api.githubCopilot.cancelPolling();
      setCopilotAuthStatus('idle');
      setCopilotUserCode('');
      setCopilotError(null);
    } catch (error) {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      let response: Awaited<ReturnType<typeof api.api.fetch>>;
              ...
              <label className="flex items-center justify-between cursor-pointer">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      const result = await api.autoLaunch.set(next);
                      if (result.success) {
              ...
              <label className="flex items-center justify-between cursor-pointer">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      const result = await api.preventSleep.set(next);
                      if (result.success) {
              ...
              <div className="flex items-center justify-between pb-2 border-b border-border">
                <div className="flex items-center gap-1.5">
                  ...
                  {ProviderRegistry.get(activeProvider)?.website && (
                    <button
                      type="button"
                      onClick={() => void api.shell.openExternal(ProviderRegistry.get(activeProvider)!.website!)}
                    ...
                    <div className="min-h-[68px]">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {ProviderRegistry.get('minimax')?.apiKeyUrl && (
                          <button
                            type="button"
                            onClick={() => void api.shell.openExternal(ProviderRegistry.get('minimax')!.apiKeyUrl!)}
                      ...
                      {minimaxOAuthPhase.kind === 'pending' && (
                        <div className="p-3 rounded-xl bg-surface-inset border border-border space-y-2">
                          ...
                          <a
                            ...
                            onClick={(e) => { e.preventDefault(); void api.shell.openExternal(minimaxOAuthPhase.verificationUri); }}
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {ProviderRegistry.get(activeProvider)?.apiKeyUrl && (
                          <button
                            type="button"
                            onClick={() => void api.shell.openExternal(ProviderRegistry.get(activeProvider)!.apiKeyUrl!)}
                            className="text-[11px] text-claude-accent hover:underline transition-colors"
                          >
                            {i18nService.t('getApiKey')} →
                          </button>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {ProviderRegistry.get('qwen')?.apiKeyUrl && (
                          <button
                            type="button"
                            onClick={() => void api.shell.openExternal(ProviderRegistry.get('qwen')!.apiKeyUrl!)}
                            className="text-[11px] text-claude-accent hover:underline transition-colors"
                          >
                            {i18nService.t('getApiKey')} →
                          </button>
                  ...
                  {(copilotAuthStatus === 'awaiting_user' || copilotAuthStatus === 'polling') && (
                    <div className="space-y-3">
                      <div className="p-3 rounded-xl bg-claude-surfaceInset dark:bg-claude-darkSurfaceInset border border-claude-border dark:border-claude-darkBorder">
                        ...
                        <button
                          type="button"
                          onClick={() => api.shell.openExternal(copilotVerificationUri)}
