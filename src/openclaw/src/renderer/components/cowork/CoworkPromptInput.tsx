...
const CoworkPromptInput = React.forwardRef<CoworkPromptInputRef, CoworkPromptInputProps>(
  ...
  const saveInlineFile = useCallback(async (file: File): Promise<string | null> => {
    try {
      ...
      const result = await api.dialog.saveInlineFile({
        dataBase64,
        fileName: file.name,
        mimeType: file.type,
    ...
    for (const file of files) {
      ...
      if (fileIsImage) {
        if (modelSupportsImage) {
          ...
          if (nativePath) {
            try {
              const result = await api.dialog.readFileAsDataUrl(nativePath);
    ...
    try {
      ...
      for (const filePath of result.paths) {
        if (isImagePath(filePath)) {
          if (modelSupportsImage) {
            try {
              const readResult = await api.dialog.readFileAsDataUrl(filePath);
