  ...
  <div className="mb-3 p-3 rounded-lg border border-dashed border-border-subtle">
    ...
    {guideUrl && (
      <button
        type="button"
        onClick={() => {
          api.shell.openExternal(guideUrl).catch((err: unknown) => {
...
const IMSettings: React.FC = () => {
  ...
  useEffect(() => {
    api?.im?.getLocalIp?.().then((ip: string) => {
  ...
  const _handleFeishuStartQr = async () => {
    ...
    setFeishuQrError('');
    try {
      const result = await api.feishu.install.qrcode(false);
      ...
      feishuQrPollTimerRef.current = setInterval(async () => {
        try {
          const pollResult = await api.feishu.install.poll(feishuQrDeviceCodeRef.current);
          if (!isMountedRef.current) return;
  ...
  const handleWeixinQrLogin = async () => {
    ...
    setWeixinQrError('');
    try {
      const startResult = await api.im.weixinQrLoginStart();
      if (!isMountedRef.current) return;
      ...
      const waitResult = await api.im.weixinQrLoginWait(startResult.sessionKey);
      if (weixinTimerRef.current) { clearTimeout(weixinTimerRef.current); weixinTimerRef.current = null; }
      if (!isMountedRef.current) return;
