...
class AgentService {
  ...
  async createAgent(request: {
    ...
    try {
      const agent = await api.agents.create(request) as any;
      if (agent) {
  ...
  async updateAgent(id: string, updates: {
    ...
    try {
      const agent = await api.agents.update(id, updates) as any;
      if (agent) {
  ...
  async getPresets(): Promise<PresetAgent[]> {
    try {
      const presets = await api.agents.presets() as any;
      return presets ?? [];
    } catch (error) {
  ...
  async addPreset(presetId: string): Promise<Agent | null> {
    try {
      const agent = await api.agents.addPreset(presetId) as any;
      if (agent) {
        store.dispatch(addAgent({
          id: agent.id,
          name: agent.name,
