...
class AuthService {
  ...
  async init() {
    ...
    try {
      const result = await api.auth.getUser() as any;
  ...
  private async fetchLoginUrl(): Promise<string> {
    ...
    try {
      const response = await api.api.fetch({
        url,
        method: 'GET',
  ...
  async handleCallback(code: string) {
    try {
      const result = await api.auth.exchange(code) as any;
      if (result.success) {
        store.dispatch(setLoggedIn({ user: result.user, quota: result.quota }));
        await this.loadServerModels();
      }
  ...
  async refreshQuota() {
    try {
      const result = await api.auth.getQuota() as any;
      if (result.success) {
  ...
  async fetchProfileSummary() {
    try {
      const result = await api.auth.getProfileSummary() as any;
  ...
  async getAccessToken(): Promise<string | null> {
    try {
      return await api.auth.getAccessToken();
    } catch {
      return null;
    }
  }
  ...
  private async loadServerModels() {
    try {
      const modelsResult = await api.auth.getModels() as any;
