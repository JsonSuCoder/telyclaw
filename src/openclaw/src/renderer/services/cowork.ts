...
class CoworkService {
  ...
  async loadSessions(agentId?: string): Promise<void> {
    ...
    const result = await tauriApi.cowork.listSessions(agentId);
  ...
  async listMemoryEntries(input: {
    ...
    const api = tauriApi.cowork.listMemoryEntries;
    if (!api) return [];
  ...
  async createMemoryEntry(input: {
    ...
    const api = tauriApi.cowork.createMemoryEntry;
    if (!api) return null;
    const result = await api(input);
  ...
  async updateMemoryEntry(input: {
    ...
    const api = tauriApi.cowork.updateMemoryEntry;
    if (!api) return null;
    const result = await api(input);
    if (!result?.success || !result.entry) return null;
    return result.entry;
  ...
  async deleteMemoryEntry(input: { id: string }): Promise<boolean> {
    const api = tauriApi.cowork.deleteMemoryEntry;
    if (!api) return false;
    const result = await api(input);
  ...
  async getMemoryStats(): Promise<CoworkMemoryStats | null> {
    const api = tauriApi.cowork.getMemoryStats;
    if (!api) return null;
  ...
  async readBootstrapFile(filename: string): Promise<string> {
    const api = tauriApi.cowork.readBootstrapFile;
    if (!api) return '';
  ...
  async writeBootstrapFile(filename: string, content: string): Promise<boolean> {
    const api = tauriApi.cowork.writeBootstrapFile;
    if (!api) return false;
  ...
  async generateSessionTitle(prompt: string | null): Promise<string | null> {
    if (!tauriApi?.generateSessionTitle) {
      return null;
    }
    return tauriApi.generateSessionTitle(prompt);
  }

  async getRecentCwds(limit?: number): Promise<string[]> {
    if (!tauriApi?.getRecentCwds) {
      return [];
    }
    return tauriApi.getRecentCwds(limit);
  }
