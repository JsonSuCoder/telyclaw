...
import type { Platform } from '../../shared/platform/index';
import {
  setConfig,
  setStatus,
  setLoading,
...
class IMService {
  ...
  private async _doInit(): Promise<void> {
    ...
    this.statusUnsubscribe = api.im.onStatusChange((status: IMGatewayStatus) => {
      store.dispatch(setStatus(status));
    });

    // Set up message listener (for logging/monitoring)
    this.messageUnsubscribe = api.im.onMessageReceived((message) => {
  ...
  async loadConfig(): Promise<IMGatewayConfig | null> {
    try {
      ...
      const result: IMConfigResult = await api.im.getConfig();
  ...
  async loadStatus(): Promise<IMGatewayStatus | null> {
    try {
      const result: IMStatusResult = await api.im.getStatus();
  ...
  async updateConfig(config: Partial<IMGatewayConfig>): Promise<boolean> {
    try {
      store.dispatch(setLoading(true));
      const result: IMGatewayResult = await api.im.setConfig(config, { syncGateway: true });
      if (result.success) {
  ...
  async persistConfig(config: Partial<IMGatewayConfig>): Promise<boolean> {
    try {
      const result: IMGatewayResult = await api.im.setConfig(config, { syncGateway: false });
      if (result.success) {
        return true;
      } else {
  ...
  async saveAndSyncConfig(): Promise<boolean> {
    try {
      const result: IMGatewayResult = await api.im.syncConfig();
  ...
  async startGateway(platform: Platform): Promise<boolean> {
    try {
      ...
      const result: IMGatewayResult = await api.im.startGateway(platform);
      if (result.success) {
        await this.loadStatus();
        return true;
      } else {
  ...
  async stopGateway(platform: Platform): Promise<boolean> {
    try {
      store.dispatch(setLoading(true));
      const result: IMGatewayResult = await api.im.stopGateway(platform);
      if (result.success) {
        await this.loadStatus();
        return true;
      } else {
  ...
  async testGateway(
    ...
    try {
      store.dispatch(setLoading(true));
      const result: IMConnectivityTestResponse = await api.im.testGateway(platform, configOverride);
  ...
  async listPairingRequests(platform: string) {
    return api.im.listPairingRequests(platform);
  }
  ...
  async approvePairingCode(platform: string, code: string) {
    return api.im.approvePairingCode(platform, code);
  }
  ...
  async rejectPairingRequest(platform: string, code: string) {
    return api.im.rejectPairingRequest(platform, code);
  }
  ...
  async getOpenClawConfigSchema(): Promise<{ schema: Record<string, unknown>; uiHints: Record<string, Record<string, unknown>> } | null> {
    try {
      const result = await api.im.getOpenClawConfigSchema();
      if (result.success && result.result) {
        return result.result;
      }
      return null;
  ...
  async addDingTalkInstance(name: string): Promise<DingTalkInstanceConfig | null> {
    try {
      const result = await api.im.addDingTalkInstance(name);
  ...
  async deleteDingTalkInstance(instanceId: string): Promise<boolean> {
    try {
      const result = await api.im.deleteDingTalkInstance(instanceId);
      if (result.success) {
  ...
  async persistDingTalkInstanceConfig(instanceId: string, config: Partial<DingTalkOpenClawConfig>): Promise<boolean> {
    try {
      const result = await api.im.setDingTalkInstanceConfig(instanceId, config, { syncGateway: false });
      if (result.success) {
  ...
  async updateDingTalkInstanceConfig(instanceId: string, config: Partial<DingTalkOpenClawConfig>): Promise<boolean> {
    try {
      store.dispatch(setLoading(true));
      const result = await api.im.setDingTalkInstanceConfig(instanceId, config, { syncGateway: true });
      if (result.success) {
        await this.loadConfig();
        await this.loadStatus();
        return true;
  ...
  async addQQInstance(name: string): Promise<QQInstanceConfig | null> {
    try {
      const result = await api.im.addQQInstance(name);
      if (result.success && result.instance) {
  ...
  async deleteQQInstance(instanceId: string): Promise<boolean> {
    try {
      const result = await api.im.deleteQQInstance(instanceId);
      if (result.success) {
  ...
  async persistQQInstanceConfig(instanceId: string, config: Partial<QQOpenClawConfig>): Promise<boolean> {
    try {
      const result = await api.im.setQQInstanceConfig(instanceId, config, { syncGateway: false });
      if (result.success) {
  ...
  async updateQQInstanceConfig(instanceId: string, config: Partial<QQOpenClawConfig>): Promise<boolean> {
    try {
      store.dispatch(setLoading(true));
      const result = await api.im.setQQInstanceConfig(instanceId, config, { syncGateway: true });
      if (result.success) {
        await this.loadConfig();
        await this.loadStatus();
        return true;
  ...
  async addFeishuInstance(name: string): Promise<FeishuInstanceConfig | null> {
    try {
      const result = await api.im.addFeishuInstance(name);
      if (result.success && result.instance) {
  ...
  async deleteFeishuInstance(instanceId: string): Promise<boolean> {
    try {
      const result = await api.im.deleteFeishuInstance(instanceId);
      if (result.success) {
  ...
  async persistFeishuInstanceConfig(instanceId: string, config: Partial<FeishuOpenClawConfig>): Promise<boolean> {
    try {
      const result = await api.im.setFeishuInstanceConfig(instanceId, config, { syncGateway: false });
      if (result.success) {
  ...
  async updateFeishuInstanceConfig(instanceId: string, config: Partial<FeishuOpenClawConfig>): Promise<boolean> {
    try {
      store.dispatch(setLoading(true));
      const result = await api.im.setFeishuInstanceConfig(instanceId, config, { syncGateway: true });
      if (result.success) {
        await this.loadConfig();
        await this.loadStatus();
        return true;
