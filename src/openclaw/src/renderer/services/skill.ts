...
class SkillService {
  ...
  async loadSkills(): Promise<Skill[]> {
    try {
      const result = await api.skills.list() as any;
  ...
  async setSkillEnabled(id: string, enabled: boolean): Promise<Skill[]> {
    try {
      const result = await api.skills.setEnabled({ id, enabled }) as any;
      if (result.success && result.skills) {
        this.skills = result.skills;
        return this.skills;
      }
  ...
  async deleteSkill(id: string): Promise<{ success: boolean; skills?: Skill[]; error?: string }> {
    try {
      const result = await api.skills.delete(id) as any;
      if (result.success && result.skills) {
        this.skills = result.skills;
      }
      return result;
  ...
  async downloadSkill(source: string): Promise<{
    ...
    try {
      const result = await api.skills.download(source) as any;
      if (result.success && result.skills) {
        this.skills = result.skills;
      }
      return result;
  ...
  async confirmInstall(
    ...
    try {
      const result = await api.skills.confirmInstall(pendingId, action) as any;
      if (result.success && result.skills) {
        this.skills = result.skills;
      }
      return result;
  ...
  async upgradeSkill(skillId: string, downloadUrl: string): Promise<{
    ...
    try {
      const result = await api.skills.upgrade(skillId, downloadUrl) as any;
      if (result.success && result.skills) {
        this.skills = result.skills;
      }
      return result;
  ...
  async getSkillsRoot(): Promise<string | null> {
    try {
      const result = await api.skills.getRoot() as any;
  ...
  onSkillsChanged(callback: () => void): () => void {
    return api.skills.onChanged(callback);
  }
  ...
  async getSkillConfig(skillId: string): Promise<Record<string, string>> {
    try {
      const result = await api.skills.getConfig(skillId) as any;
  ...
  async setSkillConfig(skillId: string, config: Record<string, string>): Promise<boolean> {
    try {
      const result = await api.skills.setConfig(skillId, config) as any;
  ...
  async testEmailConnectivity(
    ...
    try {
      const result = await api.skills.testEmailConnectivity(skillId, config) as any;
  ...
  async getAutoRoutingPrompt(): Promise<string | null> {
    try {
      const result = await api.skills.autoRoutingPrompt() as any;
