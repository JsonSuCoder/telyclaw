...
export class CronJobService {
  ...
  private emitStatusUpdate(taskId: string, state: TaskState): void {
    // In Tauri, events are emitted via the backend or a sidecar protocol
    console.log('[CronJobService] emitStatusUpdate:', { taskId, state });
  }

  private emitRunUpdate(run: ScheduledTaskRunWithName): void {
    console.log('[CronJobService] emitRunUpdate:', { run });
  }

  private emitFullRefresh(): void {
    console.log('[CronJobService] emitFullRefresh');
  }
}
